...
class KyoboDirectDentalPage(BasePage):
    def __init__(self, driver, option: str):
        ...
        self.year = option
        if option == 10:
            self.year_locator = locators.TEN_YEAR_COVERAGE_BUTTON
        elif option == 20:
            self.year_locator = locators.TWENTY_YEAR_COVERAGE_BUTTON
    ...
    def scrape(self, input_pairs, ):
        ...
        # CSV 파일 생성
        file = open(
            f'{download_folder}/{__class__.__name__}_{self.year}.csv', 
            mode='w', 
            newline='', 
            encoding='UTF8'
        )
    ...
    def __gather_data(self):
        ...
        - 각 보험마다 따로 구현 필요.
        """
        year_button = self.find(self.year_locator)
        year_button.click()

        cost = self.find(locators.PLAN_COST).text
        # 쉼표를 제거하고 정수형으로 변환
